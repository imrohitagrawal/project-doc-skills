...
- `README.md` and `per-skill-review-prompt.md` are root scaffolding (never bundled), so they were
  ...
  spot let the skill **enumeration** drift twice — a stale count, and several sites missing the new
  skill (in `README.md`: the skill table, the repo tree, and the improve-order list; in
  `per-skill-review-prompt.md`: the `{SKILL_NAME}` pick-list and the attachment table) — and ship
  uncaught (all fixed in the doc-critic PR, `b65041f`). This lint derives the canonical set from
  `skills/<name>/` and asserts that **every one of those five enumeration sites** lists exactly that
  set, plus the suite count phrases (word or digit, every occurrence).
- **Low false-positive by design:** the set checks are exact directory-vs-document comparisons; the
  count checks are anchored to specific suite-count phrases, so the legitimate "the other seven
